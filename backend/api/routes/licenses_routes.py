...
def create_license(license: LicenseCreate, db: Session = Depends(require_role("user","admin"))):
...
def read_licenses(skip: int = 0, limit: int = 100, db: Session = Depends(require_role("user","admin","read"))):
    licenses = db.query(OfficeLicenses).offset(skip).limit(limit).all()
    return licenses

@router.get("/{license_id}", response_model=LicenseRead)
def read_license(license_id: int, db: Session = Depends(require_role("user","admin","read"))):
    db_license = db.query(OfficeLicenses).filter(OfficeLicenses.id == license_id).first()
    return db_license

@router.delete("/{license_id}")
def delete_license(license_id: int, db: Session = Depends(require_role("user","admin"))):
    db_license = db.query(OfficeLicenses).filter(OfficeLicenses.id == license_id).first()
    if not db_license:
...
def update_license(license_id: int, license: LicenseUpdate, db: Session = Depends(require_role("user","admin"))):
    db_license = db.query(OfficeLicenses).filter(OfficeLicenses.id == license_id).first()
    if not db_license:
        raise HTTPException(status_code=404, detail="License not found")
